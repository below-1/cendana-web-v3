      ...
      <div class="col-md-6 col-xs-12">
        <q-card 
          ...
          <q-card-section>
            <q-form ref="form">
              <inv-cat-options
                v-model="payload.investment"
                class="q-mb-md"
                :rules="[
                  v => !!v || 'Kategori Investasi Harus Diisi'
                ]"
              />
              <rupiah-input
                label="Nominal"
...
import InvCatOptions from 'components/inv-cat/invcat-options.vue'
...
function getInitialData(id: string | number) {
  const { 
    getSingleEntity
  } = useSingleEntity('Investasi')

  return getSingleEntity(`/v1/api/investment-trans/${id}`)
}
...
export default defineComponent({
  ...
  setup(props) {
    ...
    onMounted(async () => {
      ...
      payload.investment = data.investment
    ...
    const { updateEntity, result } = useUpdateEntityV2({
      entityName: 'Investasi',
      transform: (p) => {
        const { investment, createdAt, ...rest } = p
        return {
          ...rest,
    ...
    const onSubmit = async () => {
      ...
      await updateEntity(`/v1/api/investment-trans/${props.id}`, payload)
      router.back()
    }
